...
# Значения для этих столбцов также оформить в виде списка и
...
def get_data():
    ...
    main_data.append(main_data_headers)

    for data_file in files_list:
        with open(data_file, 'rb') as f:
            bd = f.read()
            f_encoding = chardet.detect(bd)['encoding']
            data_for_samples = bd.decode(f_encoding)

        os_prod_list.append(re.compile(r'Изготовитель системы:\s*\S*').findall(data_for_samples)[0].split()[2])
        os_name_list.append(re.compile(r'Windows\s*\S*').findall(data_for_samples)[0])
        os_code_list.append(re.compile(r'Код продукта:\s*\S*').findall(data_for_samples)[0].split()[2])
        os_type_list.append(re.compile(r'Тип системы:\s*\S*').findall(data_for_samples)[0].split()[2])

    data_table = [os_prod_list, os_name_list, os_code_list, os_type_list]

    for i in range(len(data_table[0])):
        row_data = [data[i] for data in data_table]
        main_data.append(row_data)

    return main_data


def write_to_csv(csv_file_name) -> object:
    import csv
